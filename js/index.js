...
function initChart(data) {
  ...
  dataTree.listen('treeItemUpdate', function(e) {
    ...
    var data = {action: 'update', id: id, field: e.field, value: e.value};
  ...
  $('#editButtonSave').click(function() {
    if ($('#actualStart').data("DateTimePicker").date() && $('#actualEnd').data("DateTimePicker")) {
      ...
      if (asValue && aeValue && nameValue) {
        ...
        var asUtc = asValue.getTime() - asValue.getTimezoneOffset() * 60000;
        var aeUtc = aeValue.getTime() - aeValue.getTimezoneOffset() * 60000;
        ...
        if (isDef(selectedTaskId)) {
          ...
          if (item) {
            ...
            item.set('actualStart', asUtc);
            item.set('actualEnd', aeUtc);
          ...
          var newItem = {id: -1, name: nameValue, actualStart: asUtc, actualEnd: aeUtc, progressValue: '0%'};
...
function updateEditForm() {
  if (isDef(selectedTaskId)) {
    ...
    //Values can be stroed in database as strings, that's why casted to numbers.
    //Also the timezone must be restored.
    var asValue = +item.get('actualStart') + date.getTimezoneOffset() * 60000;
    var aeValue = +item.get('actualEnd') + date.getTimezoneOffset() * 60000;
